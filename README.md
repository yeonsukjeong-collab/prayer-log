...
- 기도제목 목록을 목원별·기간별(전체/최근 1주/2주/1달/날짜 직접 선택)로 필터링 (기본은 전체 목원·전체 기간)
- 기도제목 카드 상단에 이름·날짜(요일)를 굵은 파란 글씨로 표시, 하이픈 항목은 들여쓴 글머리 기호 목록으로 표시
- 기도제목 목록 확인 (실제 작성자 본인만 수정·삭제 가능, 목자는 전체 관리 가능)
- 카드 상단 우측의 "수정" 버튼으로 내용 수정 + 응답 여부/응답 내용을 함께 편집
- 릴레이 기도 작성 및 목록 열람, 카드 상단에 이름·날짜(요일) 굵은 파란 글씨로 표시
- 릴레이 기도 목록도 기도제목과 동일하게 목원별·기간별로 필터링 가능
