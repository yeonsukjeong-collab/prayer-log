...
시드 데이터 기준 **정연석**이 목자로 지정되어 있습니다 (`server/prisma/seed.ts`의 `isLeader: true`). 목자로 로그인하면:

- 기도제목 등록 시 **작성자 이름을 선택**할 수 있습니다 (전화·문자 등으로 받은 기도제목을 대신 입력할 때 사용).
- 다른 사람이 작성한 기도제목도 응답 표시·삭제할 수 있습니다.

일반 목장원은 본인 이름으로만 등록되며, 다른 사람 이름으로 등록을 시도하면 서버에서 거부됩니다.
...
- 기도제목 등록 / 목록 확인 (본인 글만 수정·삭제 가능, 목자는 전체 관리 가능)
- 목자는 기도제목 등록 시 작성자를 다른 목장원으로 선택 가능 (대신 입력)
- 기도제목에 "응답됨" 표시 + 응답 내용 기록
- 대표기도문 작성 및 목록 열람
- 화면 글자 크기 조절 (가- / 가+, 브라우저에 저장됨)
