...
import type { PastorPrayer } from "../types";
import { formatDateWithWeekday, toDateInputValue } from "../utils/date";

interface Props {
  item: PastorPrayer;
  onUpdate: (id: string, data: { content?: string; prayerDate?: string }) => Promise<void>;
  onDelete: (id: string) => void;
}

export function PastorPrayerItem({ item, onUpdate, onDelete }: Props) {
  const { user } = useAuth();
  const canManage = Boolean(user?.isAdmin || user?.isLeader);

  const [expanded, setExpanded] = useState(false);
  const [editing, setEditing] = useState(false);
  const [editContent, setEditContent] = useState(item.content);
  const [editDate, setEditDate] = useState(() => toDateInputValue(new Date(item.prayerDate)));
  const [saving, setSaving] = useState(false);

  const { dateLabel, weekdayLabel } = formatDateWithWeekday(new Date(item.prayerDate));

  function startEditing() {
    setEditContent(item.content);
    setEditDate(toDateInputValue(new Date(item.prayerDate)));
    setEditing(true);
    setExpanded(true);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await onUpdate(item.id, { content: trimmed, prayerDate: editDate });
      setEditing(false);
    } finally {
      setSaving(false);
    }
